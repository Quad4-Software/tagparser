## [2.1.0](https://git.quad4.io/Go-Libs/tagparser) (2026-04-18)

Quad4 fork: module path, layout, toolchain, CI, performance, and tests.

This is the first **git tag** whose `go.mod` declares `git.quad4.io/Go-Libs/tagparser/v2`, so consumers can depend on **`v2.1.0`** with plain `go get`.

### Module and imports

- Module path is `git.quad4.io/Go-Libs/tagparser/v2`.
- Import the API as `git.quad4.io/Go-Libs/tagparser/v2/pkg/tagparser`.
...
- **Property-based** via [pbt](https://git.quad4.io/Go-Libs/pbt): `pbt_test.go` (no panic, determinism, `HasOption` matches `Options`).
